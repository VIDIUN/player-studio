...
VMCModule.config(['$routeProvider', '$locationProvider', '$httpProvider', '$tooltipProvider', '$translateProvider', function ($routeProvider, $locationProvider, $httpProvider, $tooltipProvider, $translateProvider) {
	...
	var vmc = window.parent.vmc;
    if (vmc && vmc.vars.studioV3.showFlashStudio === false && vmc.vars.studioV3.showHTMLStudio === false){
        $(".vmcSubNav").hide();
    }
    if (vmc && vmc.vars.studioV3.showFlashStudio === false){
        $("#flashStudioBtn").hide();
    }
    if (vmc && vmc.vars.studioV3.showHTMLStudio === false){
        $("#htmlStudioBtn").hide();
    }
