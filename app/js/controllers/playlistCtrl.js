VMCMenu.controller('playlistCtrl', ['$scope', '$modal', 'utilsSvc',
	function ($scope, $modal, utilsSvc) {
		...
		for (var i = 0; i < $scope.plugin.properties.length; i++) {
			...
			if (prop.model.indexOf("config.plugins.playlistAPI.vpl") === 0 && prop.model.indexOf("Id") === prop.model.length - 2) {
				var playlistID = prop.initvalue;
				var playlistName = $scope.plugin.properties[i + 1].initvalue;
				var editable = (prop.model.indexOf("config.plugins.playlistAPI.vpl0Id") === -1);
		...
		if ($scope.playerData.config.plugins && $scope.playerData.config.plugins.playlistAPI) {
			$scope.additionalPlaylists[0] = {"id": $scope.playerData.config.plugins.playlistAPI.vpl0Id, "name": $scope.playerData.config.plugins.playlistAPI.vpl0Name, "editable": false};
		}
		...
		$scope.updatePlaylist = function (prop, trigger) {
			...
			for (var i = $scope.plugin.properties.length - 1; i >= 0; i--) {
				...
				if (p.model.indexOf("config.plugins.playlistAPI.vpl") === 0) { // this is a playlist property
			...
			for (var playlistIndex = 0; playlistIndex < $scope.additionalPlaylists.length; playlistIndex++) {
				$scope.plugin.properties.push({"model": "config.plugins.playlistAPI.vpl" + playlistIndex + "Id", "initvalue": $scope.additionalPlaylists[playlistIndex].id, "type": "hiddenValue"});
				$scope.plugin.properties.push({"model": "config.plugins.playlistAPI.vpl" + playlistIndex + "Name", "initvalue": $scope.additionalPlaylists[playlistIndex].name, "type": "hiddenValue"});
			}

			// remove redundant playlists from playerData
			if ($scope.playerData.config.plugins && $scope.playerData.config.plugins.playlistAPI) {
				var pData = $scope.playerData.config.plugins.playlistAPI;
				if (pData["vpl" + $scope.additionalPlaylists.length + "Id"]) {
					delete pData["vpl" + $scope.additionalPlaylists.length + "Id"];
					delete pData["vpl" + $scope.additionalPlaylists.length + "Name"];
				}
			}
